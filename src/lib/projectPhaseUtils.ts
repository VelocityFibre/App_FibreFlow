...
export async function autoAssignFirstPhaseAndTasks({
  ...
  if (phaseAssigneeId !== null && phaseAssigneeId !== undefined && phaseAssigneeId !== '') {
    const { data: staffData, error: staffError } = await supabase
      .from("staff")
      .select("id")
      .eq("id", phaseAssigneeId)
      .single();
    
    if (staffError || !staffData) {
      console.error(`Invalid phase assignee ID ${phaseAssigneeId}:`, staffError);
      // Clear invalid assignee ID instead of throwing error
      phaseAssigneeId = null;
    }
  }
  
  if (taskAssigneeId !== null && taskAssigneeId !== undefined && taskAssigneeId !== '') {
    const { data: staffData, error: staffError } = await supabase
      .from("staff")
      .select("id")
    ...
    if (staffError || !staffData) {
      console.error(`Invalid task assignee ID ${taskAssigneeId}:`, staffError);
      // Clear invalid assignee ID instead of throwing error
      taskAssigneeId = null;
    }
  }
